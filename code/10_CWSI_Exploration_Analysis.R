...
# read in irrigation demand (min. rel. TWD)
irrig <- read_csv("./data/processed/Min_rel_TWD_df.csv") 
...
irrig_CWSI <- full_join(CWSI_df, irrig, by = c("Tree.ID",  "Date")) 
...
irrig_CWSI <- irrig_CWSI %>%
  dplyr::select(-c(Tree_ID, DateTime, min_rel_TWD)) %>%
...
r2_df <- cwsi_solarnoon %>%
  ...
  dplyr::select(R2, p_value, R2_label)
